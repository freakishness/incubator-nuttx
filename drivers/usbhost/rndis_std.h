...
#define RNDIS_INDICATE_STATUS_MSG            0x00000040
...
#define NDIS_HW_STATUS_READY                 0x00000000
#define NDIS_HW_STATUS_INITIALIZING          0x00000001
#define NDIS_HW_STATUS_RESET                 0x00000002
#define NDIS_HW_STATUS_CLOSING               0x00000003
#define NDIS_HW_STATUS_NOT_READY             0x00000004
...
#define NDIS_MEDIUM_WIFI                     0x00000008
#define NDIS_MEDIUM_CO_WAN                   0x00000009
#define NDIS_MEDIUM_IRDA                     0x0000000c
#define NDIS_MEDIUM_BPC                      0x0000000d
#define NDIS_MEDIUM_IP_TUNNEL                0x0000000e
#define NDIS_MEDIUM_NATIVE_802_11            0x0000000f
#define NDIS_MEDIUM_LOOPBACK                 0x00000010
...
struct rndis_query_msg
{
  ...
  uint32_t buffer_len;           /* InformationBufferLength */
  uint32_t buffer_offset;        /* InformationBufferOffset */
  uint32_t device_vc_handle;     /* Device Vc Handle */
  uint32_t buffer[];
};
...
struct rndis_query_cmplt
{
  ...
  uint32_t buffer_len;            /* InformationBufferLength */
  uint32_t buffer_offset;         /* InformationBufferOffset */
  uint32_t buffer[];              /* Buffer Offset */
};

struct rndis_set_msg_s
{
  struct rndis_command_header hdr;
  uint32_t objid;         /* ObjectID */
  uint32_t buflen;        /* InformationBufferLength */
  uint32_t bufoffset;     /* InformationBufferOffset */
  uint32_t device_vc_handle;      /* Reserved */
  uint32_t buffer[];      /* Buffer */
};

struct rndis_set_cmplt_s
{
  struct rndis_response_header hdr;
};
...
struct rndis_keepalive_msg_s
{
  struct rndis_command_header hdr;
};

struct rndis_keepalive_cmplt_s
{
  struct rndis_response_header hdr;
};

struct rndis_indicate_msg_s
{
  uint32_t msgtype;       /* MessageType */
  uint32_t msglen;        /* MessageLength */
  uint32_t status;        /* Status */
...
#define RNDIS_SIZEOF_NOTIFICATION_S(n) (sizeof(struct rndis_indicate_msg_s) + (n))
