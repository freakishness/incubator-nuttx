/****************************************************************************
 * boards/arm/imxrt/ok1050-c/src/imxrt_flexspi_nor.c
 *
...
static uint32_t g_flexspi_nor_lut[][4] =
{
  [READ_NORMAL] =
  {
    ...
    FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_READ_SDR,   FLEXSPI_1PAD, 0x04,
  ...
  [READ_STATUS] =
  {
    FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR,        FLEXSPI_1PAD, 0x81,
                    FLEXSPI_COMMAND_RADDR_SDR,  FLEXSPI_1PAD, 0x04),
  },
  [WRITE_ENABLE] =
  {
  ...
  [EXIT_QPI] =
  {
    FLEXSPI_LUT_SEQ(FLEXSPI_COMMAND_SDR,        FLEXSPI_4PAD, 0xf5,
                    FLEXSPI_COMMAND_STOP,       FLEXSPI_1PAD, 0x00),
  },
  [READ_STATUS_REG] =
  {
...
static struct flexspi_device_config_s
g_flexspi_device_config __attribute__((section(".data"))) =
{
...
int imxrt_flexspi_nor_initialize(void)
{
  ...
  FLEXSPI_SOFTWARE_RESET(g_flexspi_nor_b.flexspi);

  if (imxrt_flexspi_nor_get_vendor_id(&g_flexspi_nor_b, &vendor_id))
    {
      return -EIO;
    }
  ...
  part[0] = mtd_partition(master, 0, 4096);
  if (!part[0])
    {
  ...
  part[1] = mtd_partition(master, 4096, 20480);
  if (!part[1])
    {
      syslog(LOG_ERR, "make partition failed. offset = 0, nblocks = 4096");
      return -1;
  ...
  part[2] = mtd_partition(master, 24576, 20480);
  if (!part[2])
    {
      syslog(LOG_ERR, "make partition failed. offset = 0, nblocks = 4096");
      return -1;
  ...
  part[3] = mtd_partition(master, 45056, 4096);
  if (!part[3])
    {
      syslog(LOG_ERR, "make partition failed. offset = 0, nblocks = 4096");
      return -1;
  ...
  part[4] = mtd_partition(master, 49152, 16384);
  if (!part[4])
    {
      syslog(LOG_ERR, "make partition failed. offset = 0, nblocks = 4096");
      return -1;
